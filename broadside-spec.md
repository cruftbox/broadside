...
- **Bluesky link shortening.** Bluesky's own app never posts a URL's literal typed text: before every post it rewrites each link's *visible* text to `host + truncated-path + "..."` (scheme stripped; path+query+fragment kept to 13 characters plus an ellipsis if longer than 15), while the link still points at the full URL via the facet's `uri`. It does this unconditionally, and its own character counter counts the *shortened* length. If any Bluesky account is selected, the counter and the enforced limit must apply this same shortening to any URL in the text before counting — otherwise a post bsky.app would accept gets blocked here as "over 300" for counting the URL at its full typed length instead of its actual posted length. This does not apply when only Mastodon accounts are selected: Mastodon does not shorten links, so its count is the literal typed length.
...
- Link shortening: after computing link facets from the typed text, shorten each link facet's visible text the way bsky.app itself does (section 7) before building the record — update the facet's byte range to the shortened span and splice the shortened text into the post body, while leaving the facet's `uri` (the real link target) untouched. The `text` field actually sent to `createRecord` is this shortened text, not the literal typed text. This is what makes a post with a long URL that fits after shortening actually post, instead of being rejected by the 300-grapheme server-side limit for exceeding it at the full typed length.
